export function setCookie(name, value, expire, domain) {
	let Days = 1,
		exp = new Date();
	...
	/*document.cookie = name + "=" + encodeURI(value) + ";expires=" + exp.toGMTString() + `${name === 'mini' ? ';domain=ncwxapp.tairanmall.com' : ';domain=.tairanmall.com'}` + ";path=/";*/
}
...
/*export function clearCookie(name) {
 ...
 document.cookie = name + "=" + encodeURI('') + ";expires=" + exp.toGMTString() + ';domain=' + domain + ';path=/';
 }*/
...
export function clearCookie(name) {
	...
	for (let k in obj) {
		...
		if (k === name) {  //清除指定域下的cookie
			...
			document.cookie = name + "=" + encodeURI('') + ";expires=" + exp.toGMTString() + ';domain=ncwxapp.tairanmall.com;path=/';
