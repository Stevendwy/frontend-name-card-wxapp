			...
			<div className="cart-toTop">
				<ul>
					<a onClick={(e)=>{
						e.preventDefault();
						miniProgramShopCart()
					}}><li className="cart">{count?<span className="cart-count">{count}</span>:''}</li></a>
...
export class ShopCollection extends Component{
	...
	getShopData = ()=> {
		let self = this;
		axios.request("/ncxcx/user/getShopCollectionList?page=1").then(({data})=>{
			if(data.code===0){
				self.setState({
					update: true,
		...
		// $.ajax({
	    //     url: "/ncxcx/user/getShopCollectionList?page=1&limit=10",
	    //     type: "get",
	    //     success(data){
	    //     	if(data.code===0){
	    //     		self.setState({
	...
	addShopData = ()=>{
		...
		let self = this;
		axios.request(`/ncxcx/user/getShopCollectionList?page=${page++}`).then(({data})=>{
			if(data.code===0){
		...
		// $.ajax({
		// 	url: `/ncxcx/getShopCollectionList?page=${page++}&limit=10`,
		// 	type: 'GET',
		// 	success: function(data){
		// 		if(data.code===0){
		// 			if(data.data.data.length){
