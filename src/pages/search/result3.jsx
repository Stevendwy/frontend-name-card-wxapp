...
import { SearchBarA, LoadingRound, Shady, SearchNone, FilterNone, NetError,FastGuide} from 'component/common';
...
class SearchResult extends Component {
    ...
    if (JSON.stringify(this.props.searchData) !== JSON.stringify(nextProps.searchData)) {
      ...
      if(!this.props.hasSetSearchParams) {
        this.props.setData({ // 获取所有选项信息
          search_keywords: data.search_keywords,
          page: 1
        }, false, true).then(() => {
          this.props.dispatch(createActions('setSearchParams'));
          this.props.setData(data, true, false)
        })
      } else {
...
class SearchInput extends Component {
    ...
    browserHistory.replace(`${DIR}/search`);
	};
	//商品类型转换
	changeType = () => {
		let that = this;
...
class GoodsList extends Component {
	...
	promotionTags = () => {
		...
		if (promotion.type === 'FullDiscount') {//满折
			...
			// 	return <span className="proTips" key={index}> <span className="pro_x">{item.full}</span>件<span className="pro_y">{item.percent/10}</span>折,</span>
			// })
			return <p className="promotionTitle">以下商品 <span className="promotionRed">{promotion.title}</span></p>
...
export function searchResultDispatch(dispatch) {
	return {
		...
		setData(data, init, filterUpdate) {
			...
			return currentApi.search(data).then(result => {
				if (result.data.code !== 0) {
