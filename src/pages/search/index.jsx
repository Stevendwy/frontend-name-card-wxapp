...
import {DIR,WXAPI} from 'config/index';
import  './search.scss';

// const pageApi = {
// 	completion: {url: "/wxapi/suggests.api", method: "get"},
// 	// completion:{ url:"/src/json/suggests.json", method:"get" },
// 	hot: {url: "/wxapi/hots.api", method: "get"},
// 	placeHolder:{url:"/wxapi/placeHolder.api",method:"get"}
// };

const pageApi = {
	completion: {url: `${WXAPI}/search/auto-complete`, method: "get"},
	// completion:{ url:"/src/json/suggests.json", method:"get" },
	hot: {url: `${WXAPI}/search/hotwords`, method: "get",params:{terminal:8}},
	placeHolder:{url:`${WXAPI}/search/placeholder`,method:"get",params:{terminal:8}}
};
...
class SearchIndex extends Component {
	...
	searchSubmit = (key,link,param) => {
		...
		if( link ){
			...
			browserHistory.replace(`${DIR}/searchResult?key=${ encodeURIComponent(key)}${(param&&param.shop)?'&shop='+param.shop:''}${(param&&param.coupon_id)?'&coupon_id='+param.coupon_id:''}`);
		}
	};
	...
	render() {
		let {keyWord,param,placeHolder,searchHistory,hotList,prompt,promptClose,compList} = this.props;
		let {text,link}=this.props.location.query;
		let currentPlaceHolder = (text||link)?{word:text,link:link}:placeHolder;
			...
			{ data.slice(0,10).map((item, i) => {
				return (
					<div className="grid-key" key={i} onClick={ (e) => {
						...
						<div className={item.highlight ? "red-btn" : "black-btn"}>
							<div>{item.word }</div>
						</div>
					</div>
				)
