...
class Application extends Component {
	componentWillMount() {
		let token = getQueryString('token'),
			...
			commissionUcenterLoginType = getQueryString('commissionUcenterLoginType'),
			commissionUcenterId = getQueryString('commissionUcenterId'),
			salesmanId = getQueryString('salesmanId');
		...
		if (commissionUcenterLoginType) {
			storage.getItem("commissionUcenterLoginType") && storage.removeItem("commissionUcenterLoginType");
			storage.setItem("commissionUcenterLoginType", commissionUcenterLoginType)
		}

		if (commissionUcenterId) {
			storage.getItem("commissionUcenterId") && storage.removeItem("commissionUcenterId");
			storage.setItem("commissionUcenterId", commissionUcenterId)
		}

		if (salesmanId) {
			storage.getItem("salesmanId") && storage.removeItem("salesmanId");
			storage.setItem("salesmanId", salesmanId)
		}

		if (location.pathname == "/item") {//商品详情页
			//loadScript(COLLECTURL, sdkUse, "");//埋点存userId
			token!=="null" && this.props.getLogin();  //未登录状态下token为null,不去校验登录状态
		} else if(location.pathname == "/groupDetail"){//拼团详情页
			token!=="null" && token !== null  && this.props.getLogin();
		}else {
			this.props.getLogin();
		}
			...
			childRoutes: [
				...
				/*收货信息*/
				{
					...
					childRoutes: [
						...
						{
							path: "addressManage",
							getComponent: require('src/route/addressManage').default,
							onEnter: () => {
								shareMiniProgramUrl(location.href)
							}
						},
				...
				/*商品收藏搜索*/
				{
					path: "/goodCollectionSearch",
					getComponent: require('src/route/goodCollectionSearch').default,
					onEnter: () => {
						shareMiniProgramUrl(location.href)
					}
				},
