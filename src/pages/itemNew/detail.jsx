...
import {dateUtil, timeCtrl, judgeIphoneX, addImageSuffix, hideFlag} from "src/js/util/index"
import {miniProgramLogin, miniProgramShopCart, shareMiddlePage} from 'js/util/miniProgramLogin';
import {setExtraInfoGwc , setExtraInfoScj} from 'js/util/dcpExtraInfo';
...
export let pageApi = {
	...
	MoreGroupBuyList: {url: `${WXAPI}/item/moreGroupBuyList`, method: "get"}, //更多拼团数据
};
...
export class Tag extends Component {
	getList() {
		...
		{
			hideFlag && (biz_model === 1 ? ret.push(<li key='trc'><img src="/src/img/icon/trc-logo-icon.png"/> 泰然城自营
			</li>) : ret.push(<li
				key='trc'><img
		...
		return (
			<div>
				<div className="goods-promotion" onClick={() => {
					...
					{hideFlag && <i className="icon icon-forward vertical-middle"><img
				...
				<div className="coupon-li-wrap">
					...
					<div className="c-fr coupon-right">
						<h3>{CouponType.title(itemdata)}</h3>
							<LinkChange BackFlag={BackFlag} flag={true} className="buy-button" onClick={(e) => {
...
export class SeverArea extends PureComponent {
	render() {
		return (
			<div className="c-pb10 c-pt10">
				{hideFlag && <Add/>}
				{hideFlag && <Charge {...this.props}/>}
				<Goods {...this.props} />
			</div>
		)
	}
			...
			return <li key={i} className="rec-item" style={(i + 1) % 3 === 0 ? {marginRight: "0px"} : {}}>
				<Link to={`/item?item_id=${data.item_id}`}>
					<div className="img-wraper">
						<img
							src={data.images[0] && addImageSuffix(data.images[0], "_m") || '/src/img/item/no-img.png'}/>
	...
	onCollect = () => {
		...
		if (!this.locked()) {
			...
			axios.request(action).then(
				result => {
					this.props.ChangeCollectStatus(favorite?0:1);
					this.locked(false) //请求完成执行
                    //埋点操作
                    let {item_id,title} = this.props.data;
                    let extraInfo = setExtraInfoScj(item_id, title, null, result.status, result.data.code, '商品加入收藏成功');
                    !favorite ? sdk.dispatch('click', document.getElementById('add-collect'), extraInfo) : null;
				}
			).catch(error => {
                ...
                this.locked(false)  //请求完成执行
                //埋点操作
                let {item_id} = this.props.data;
                let extraInfo = setExtraInfoScj(item_id, title, null, status, data.code, data.message);
                !favorite ? sdk.dispatch('click', document.getElementById('add-collect'), extraInfo) : null;
			})
		}
	};
		...
		return (
			<div className="collect">
				<LinkChange className={favorite ? "save curr uncollect-goods" : "save collect-goods"}
					...
					{
						favorite ? [<span className="icon icon-favor text_sc" key="0" id="add-collect"><img
							src="/src/img/icon/collected-icon.png"/></span>,
							<span className="icon-title" key="1">已收藏</span>] :
							[<span className="shoucang" key="0" id="add-collect"><img src="/src/img/icon/collect-icon.png"/></span>,
...
class BuyModalInfo extends Component {
	...
	render() {
		let {addMinNum, changeNum, importNum, promotionData} = this.props;
		let {activity_type} = this.props.promotion;
			...
			<div className="attr-wrap">
				...
				<div className="buy-amount">
					...
					<input type="number" className="action-quantity-input"
						...
						<span
							className={`btn btn-action action-increase ${num === storeNum || num === "" || (limitNum === null ? false : num >= limitNum) || storeNum === 0 ? "dis-click" : ""}`}
							onTouchTap={num === storeNum || num === "" || (limitNum === null ? false : num >= limitNum) || storeNum === 0 ? "" : () => addMinNum(1)}>＋</span>
					</span>
...
export class BuyModalButton extends Component {
	render() {
		let {buyActive, onClickCart, onClickBuy, purchaseLimit, toCartClick, isNonPayment} = this.props;
...
export class ExpectTax extends Component {
	...
	getRate() {
		...
		let rate = (Math.ceil((+data.tax_rate).toFixed(5) * 10000) / 100).toFixed(2);
...
export class BuyModal extends Component {
	...
	importNum = (e) => {
		...
		let {num, storeNum, selectArr, nowSkuId, nowPrice} = this.props.retState;
		if (!val) {
	...
	//更改num
	changeNum = (e) => {
		let {num, nowSku, nowSkuId, nowPrice, selectArr, storeNum} = this.props.retState;
		let val = e.target.value;
		this.preNum = num;
		val = val.replace(/[^\d]/g, '');
		if (+val) {
...
export class SharingMakes extends Component {
	sharingMakesMoney = () => {
		let {item_id,commission_min_price} = this.props;
		/*let {search,origin,pathname} = window.location;
		let newArr = [];
		search.split("&").map((item, i) => {
			if (item.indexOf("token") < 0) {
				newArr.push(item)
			}
		});
		newArr=newArr.join("&");*/
		shareMiddlePage(item_id,commission_min_price);
	};

	render() {
		return <ul className="sharing-makes" onClick={this.sharingMakesMoney}>
			<li>分享赚{Number(this.props.commission_income).toFixed(2)}元</li>
			<li></li>
			<li className="c-fs10">立即<br/>分享</li>
			<li></li>
		</ul>
	}
}
