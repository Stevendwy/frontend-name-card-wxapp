...
class ItemPage extends Component {
	...
	componentWillMount() {
		const mini = this.props.location.query.mini || getCookie('mini');
		...
		let {item_id , salesmanId} = this.props.location.query;
		...
		if (item_id != this.props.data.item_id || this.props.loginBack === "loginBack") {
			...
			this.props.Promotion(item_id , salesmanId);
			this.props.Rate(item_id);
			this.props.isLogin && this.props.GetCart();
		}

		//存商品标题到sessionStorage中，供话务系统调用
		window.sessionStorage.setItem("dcpPageTitle", this.props.data && this.props.data.title);
	...
	componentWillReceiveProps(newProps) {
		let {item_id: newItem_id,salesmanId} = newProps.location.query;
		...
		if (newItem_id != item_id) {
			...
			this.props.Promotion(newItem_id , salesmanId);
...
export function ItemPageDispatch(dispatch, props) {
	return {
		...
		Promotion: (item_id , salesmanId) => {
			let _salesmanId = salesmanId ? salesmanId : window.localStorage.getItem("salesmanId");
			dispatch(axiosCreator('promotionData', {...pageApi.Promotion, params: {"item_id": item_id,"salesman_id": _salesmanId}}));
		},
		Rate: (item_id) => {
